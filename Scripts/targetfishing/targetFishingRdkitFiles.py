...
def writeProteins(compound, proteins):
    ...
    fileP = open('/home/kegg/resultadosTF/'+compound.nameCompoundKegg+'.csv', 'w')
    fileResultados = open ('/home/kegg/resultadosTF/values_'+compound.nameCompoundKegg+'.csv','w')
    maxValue = 0.9
    c3Value= 0.55
    firstLine = compound.nameCompoundKegg + ';' + compound.chemblid + ';'+compound.smile + ';'+compound.name
    fileP.write(firstLine)
    fileResultados.write(firstLine)
	...
	infoProtein = str(tid) + ';'+str(gene)+';'+targetType+';'+prefName+';'+accession
		...
		fileResultados.write(infoProtein+';'+str(maxV)+';'+str(c3)+'\n')
